...
"   to ignore it explicitely.
":match Invisible /\r$/
syntax match log_doslf "[\r]$" conceal
...
syntax region log_meta       start="^\[" end="\[\[\r\?$" contained oneline contains=log_time,log_gf,log_level,log_level_rest,log_doslf2 transparent
syntax match  log_level    "\[\(ERROR\|WARNING\|WARNG\|X\?INFO\d\?\|CONFIG\|FINE\|FINER\|FINEST\)\]" contained transparent contains=log_rest_trick,log_doslf2 nextgroup=log_rest_trick

syntax region log_error      start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[ERROR\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_warn       start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[\(WARNING\|WARNG\)\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_info       start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[X\?INFO\d\?\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_config     start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[CONFIG\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_fine       start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[FINE\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_finer      start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[FINER\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
syntax region log_finest     start="^\[\d\d\d\d-\d\d-\d\dT\d\d[:.]\d\d[:.]\d\d[.]\d\d\d[+-]\d\d\d\d\] \[[^]]\+\] \[FINEST\].*\[\[\r\?$" end="\]\]\r\?$" contains=log_meta,log_message,log_doslf
